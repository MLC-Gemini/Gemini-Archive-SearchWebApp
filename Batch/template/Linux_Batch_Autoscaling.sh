...
#restart all EC2 by killing them.
 asg_group_name=$(aws cloudformation describe-stack-resources --stack-name GEMINI-WEB-$Name-Stack| jq -r '.StackResources[]|select (.LogicalResourceId=="AutoScalingGroup").PhysicalResourceId'|sed 's/ /,/g')
 ...
 aws ec2 terminate-instances --instance-ids $(aws ec2 describe-instances --instance-id  \
                 $(aws autoscaling describe-auto-scaling-groups  --auto-scaling-group-names  \
                         $(aws cloudformation describe-stack-resources --stack-name GEMINI-WEB-$Name-Stack \
