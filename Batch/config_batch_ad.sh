...
echo "%$parent_domain-$child_domain-Delegated-SRVGEMBATCH-SudoRoot ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/01_service_accounts
echo "%$parent_domain-$child_domain-Delegated-SRVGEMBATCH-LogonAccess ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/20_svc_acct
echo "%$parent_domain-$child_domain-Delegated-SRVGEMBATCH-LogonAccess ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/01_service_accounts
echo "%$parent_domain-$child_domain-Delegated-SRVGEMBATCH-SudoSrv-gem-m ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/20_svc_acct
echo "%$parent_domain-$child_domain-Delegated-SRVGEMBATCH-SudoSrv-gem-m ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/01_service_accounts
