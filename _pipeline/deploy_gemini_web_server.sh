...
aws ssm put-parameter --name $AWS_PAR_BATCH_IMAGE_deploy --value $image_id --type "SecureString" --region "ap-southeast-2" --overwrite

# ######################################
echo "2. Start Autoscling group using 100% backed ami"
  ImageId=`aws ssm get-parameter --name $AWS_PAR_BATCH_IMAGE_deploy --with-decryption --region "ap-southeast-2" | grep Value | awk '{print $2}'|sed 's/"//g'|sed 's/,$//g'`
  echo $ImageId
