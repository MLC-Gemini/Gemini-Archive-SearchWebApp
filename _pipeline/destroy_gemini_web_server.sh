...
aws autoscaling resume-processes --auto-scaling-group-name $(aws cloudformation describe-stacks --stack-name GEMINI-WEB-$env_id-Stack | jq -r '.Stacks[].Outputs[]|select (.OutputKey=="AutoScalingGroup").OutputValue')
